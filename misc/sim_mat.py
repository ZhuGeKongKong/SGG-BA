...
import json
fg_matrix = np.load('misc/fg_matrix.npy')
fg_matrix[:,:,0] = 0
fg_matrix[0,:,:] = 0
fg_matrix[:,0,:] = 0
fg_matrix[0,0,0] = 1.0
...
rel_classes = list(pred2id.keys())
rel_classes.append("__background__")
...
def sub_obj_overlap_set(rel_classes):
    pred_dist = {}
    ...
    pred_dist_array_t = np.zeros([len(rel_classes),len(rel_classes)])
    for i in range(len(rel_classes)):
        for j in range(len(rel_classes)):
            rel_i = rel_classes[i]
            rel_j = rel_classes[j]
            if rel_i == "__background__":
                ind_i = 0
            else:
                ind_i = pred2ind[rel_i]
            if rel_j == "__background__":
                ind_j = 0
            else:
                ind_j = pred2ind[rel_j]
            pred_dist_array_t[i, j] = pred_dist_array[ind_i, ind_j]
    np.save('misc/pred_sooverlapset.npy', pred_dist_array)
    pred_dist_array = pred_dist_array_t
...
if __name__ == "__main__":
    ...
    #sub_obj_overlap()
    #sub_obj_overlap_set()
    sub_obj_overlap_set(rel_classes=rel_classes)
    # countij_cover()
    #so_entropy_pred()
