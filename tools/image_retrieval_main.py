...
#from maskrcnn_benchmark.image_retrieval.evaluation import evaluator_dot_product as evaluator
from maskrcnn_benchmark.image_retrieval.evaluation import evaluator_l1_distance as evaluator
#from maskrcnn_benchmark.image_retrieval.evaluation import evaluator_cosine_similarity as evaluator
...
# sg_train_path = model_path+model_name+'/inference/sentence2graph/sen2graph_wobg.pytorch'
# sg_test_path = model_path+model_name+'/inference_test/sentence2graph/sen2graph_wobg.pytorch'
# output_path = model_path+model_name+'/sentence2graph_wobg/sentence_retrieval_model'+'_output_%s_%d.pytorch'
# sg_train_path = model_path+model_name+'/inference/sentence2graph_new/sen2graph_wbg.pytorch'
# sg_test_path = model_path+model_name+'/inference_test/sentence2graph_new/sen2graph_wbg.pytorch'
# sg_train_path = model_path+model_name+'/inference/sentence2graph_new/sen2graph_full.pytorch'
# sg_test_path = model_path+model_name+'/inference_test/sentence2graph_new/sen2graph_full.pytorch'
sg_train_path = model_path+model_name+'/inference/sentence2graph_new/sen2graph_filter1e3.pytorch'
sg_test_path = model_path+model_name+'/inference_test/sentence2graph_new/sen2graph_filter1e3.pytorch'

print("sg_train_path: ",sg_train_path)
print("sg_test_path: ",sg_test_path)
train_data = json.load(open(sg_train_path, 'r'))
test_data = json.load(open(sg_test_path, 'r'))
sg_data = json.load(open(sg_train_path, 'r'))
sg_data.update(test_data)

train_ids = list(train_data.keys())
print('number of training:', len(train_ids))
test_ids = list(test_data.keys())
print('number of test:', len(test_ids))
...
def train(cfg, local_rank, distributed, logger):
    output_path = cfg.OUTPUT_DIR+'/sentence_retrieval_model'+'_output_%s_%d.pytorch'
    ...
    if cfg.SOLVER.PRE_VAL:
        ...
        logger.info("Test before training")
    ...
    for epoch in range(cfg.SOLVER.MAX_ITER):
        ...
        for iteration, (fg_imgs, fg_txts, bg_imgs, bg_txts) in enumerate(train_data_loader):
            ...
            for fg_img, fg_txt, bg_img, bg_txt in zip(fg_imgs, fg_txts, bg_imgs, bg_txts):
                ...
                if fg_img['relations'].size(0) != 0 and fg_txt['relations'].size(0) != 0  \
                    and bg_img['relations'].size(0) != 0 and bg_txt['relations'].size(0) != 0:
                    con_flag = 0
            if con_flag == 1:
                logger.info("Skip iteration: {iteration}".format(iteration=iteration))
                continue
            ...
            optimizer.step()
            if iteration % 200 == 0:
                logger.info("iteration: {iteration} loss: {loss:.6f} lr: {lr:.6f}".format(iteration=iteration, loss=float(
                    sum(epoch_loss) / len(epoch_loss)), lr=optimizer.param_groups[-1]["lr"]))
