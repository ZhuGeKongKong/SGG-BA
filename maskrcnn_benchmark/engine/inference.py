...
def inference(
    ...
    if cfg.TEST.CUSTUM_EVAL:
        ...
        with open(os.path.join(cfg.OUTPUT_DIR, 'custom_prediction.json'), 'w') as outfile:
            json.dump(detected_sgg, outfile)
        print('=====> ' + str(os.path.join(cfg.OUTPUT_DIR, 'custom_prediction.json')) + ' SAVED !')
        return -1.0
...
def custom_sgg_post_precessing(predictions):
    ...
    for idx, boxlist in enumerate(predictions):
        ...
        #rel_sortedid, _ = get_sorted_bbox_mapping(boxlist.get_field('pred_rel_scores_one').tolist())
        # sorted rel
        rel_pairs = []
        rel_labels = []
        rel_scores = []
        #rel_all_scores = []
        max_num_triples = min(128, len(boxlist.get_field('pred_rel_labels')))
        for i in range(max_num_triples):
            rel_labels.append(boxlist.get_field('pred_rel_labels')[i].item())
            rel_scores.append(boxlist.get_field('pred_rel_scores_one')[i].item())
