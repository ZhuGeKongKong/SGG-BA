...
detected_path = model_path+model_name+'/inference_test/VG_stanford_filtered_with_attribute_test/'
# where to save the generated annotation
output_path = model_path+model_name+'/inference_test/sentence2graph_new/sen2graph_filter1e2.pytorch'
#

data_path = './datasets/vg/'

cap_graph = json.load(open(data_path+'vg_capgraphs_anno.json'))
vg_data = h5py.File(data_path+'VG-SGG-with-attri.h5', 'r')
vg_dict = json.load(open(data_path+'VG-SGG-dicts-with-attri.json'))
vg_info = json.load(open(data_path+'image_data.json'))
...
def generate_detect_sg(det_result, det_info, valid_ids, img_coco_map, obj_thres=0.1):
    ...
    for i in range(num_img):
        ...
        if int(image_id) not in valid_ids:
            continue
        all_obj_labels = predictions[i].get_field('pred_labels').long()
        all_obj_scores = predictions[i].get_field('pred_scores')
        all_rel_pairs = predictions[i].get_field('rel_pair_idxs').long()
        ...
        rel_mask = ((all_rel_labels > 0) * (triplet_score > 0) * (all_rel_scores > 0.01)) > 0
        ...
        pred_rels = [[i[0], i[1], cap_graph['idx_to_meta_predicate'][str(j)]] for i, j in
                     zip(filter_pair.tolist(), filter_rel.tolist())]
    ...
    for i in range(num_img):
        ...
        if cap_graph['vg_valids'][i] and (coco_id in img_sg) and (coco_id in txt_sg):
            ...
            for item in img:
                ...
                relations_list_tmp = [[num_ent + r[0], num_ent + r[1], r[2]] for r in item['relations']]
                relations_list = relations_list + relations_list_tmp
                num_ent = len(encode_img['entities'])

            image_graph = np.zeros((len(encode_img['entities']), len(encode_img['relations'])), dtype=bool)
            cnt = 0
            for re in relations_list:
                image_graph[re[0]][cnt] = 1
                image_graph[re[1]][cnt] = 1
                cnt +=1
            image_graph_list = image_graph.tolist()
            ...
            for item in txt:
                ...
                relations_list_tmp = [[num_ent + r[0], num_ent + r[1], r[2]] for r in item['relations']]
                relations_list = relations_list + relations_list_tmp
                num_ent = len(encode_txt['entities'])

            text_graph = np.zeros((len(encode_txt['entities']), len(encode_txt['relations'])), dtype=bool)
            cnt = 0
            for re in relations_list:
                text_graph[re[0]][cnt] = 1
                text_graph[re[1]][cnt] = 1
                cnt +=1
            text_graph_list = text_graph.tolist()
            ...
            txt_img_sg[coco_id] = {
                'img': encode_img,
                'image_graph': image_graph_list,
                'txt': encode_txt,
                'text_graph': text_graph_list}
