...
class ApplySingleAttention(nn.Module):
    ...
    def forward(self, v, q, atten):
        ...
        v_ = torch.matmul(v_, atten.unsqueeze(1)) #/ (atten.size(-2) ** 0.5) # batch, dim, 1, que_len
...
class SGEncode(nn.Module):
    def __init__(self, img_num_obj=151, img_num_rel=51, txt_num_obj=4460, txt_num_rel=646):
        ...
        # final_fc_1 = nn.Linear(self.hidden_dim, self.hidden_dim)
        # nn.init.xavier_normal_(final_fc_1.weight)
        # final_fc_2 = nn.Linear(self.hidden_dim, self.final_dim)
        # nn.init.xavier_normal_(final_fc_2.weight)
