...
class TransformerContext(nn.Module):
    def __init__(self, config, obj_classes, rel_classes, in_channels):
        ...
        if self.effect_analysis:
            ...
            self.register_buffer("untreated_obj_feat", torch.zeros(self.in_channels + self.embed_dim + 128))
            self.register_buffer("untreated_edg_feat", torch.zeros(self.embed_dim + self.in_channels))
    ...
    def forward(self, roi_features, proposals, logger=None, all_average=False, ctx_average=False):
        ...
        obj_pre_rep_lin = self.lin_obj(obj_pre_rep)
        obj_feats = self.context_obj(obj_pre_rep_lin, num_objs)
        ...
        if (all_average or ctx_average) and self.effect_analysis and (not self.training):
            edge_pre_rep = cat((self.untreated_edg_feat.view(1, -1).expand(batch_size, -1), obj_feats), dim=-1)
        else:
            edge_pre_rep = cat((self.obj_embed2(obj_preds), roi_features, obj_feats), -1)

        # edge context
        edge_pre_rep_lin = self.lin_edge(edge_pre_rep)
        edge_ctx = self.context_edge(edge_pre_rep_lin, num_objs)
        # memorize average feature
        if self.training and self.effect_analysis:
            self.untreated_obj_feat = self.moving_average(self.untreated_obj_feat, obj_pre_rep)
            self.untreated_edg_feat = self.moving_average(self.untreated_edg_feat, cat((self.obj_embed2(obj_preds), roi_features), -1))
