...
def obj_edge_vectors(names, wv_dir, wv_type='glove.6B', wv_dim=300):
    ...
    for i, token in enumerate(names):
        ...
        if wv_index is not None:
            ...
            # Try the mean vector
            lw_token = token.split(' ')
            vector_t = []
            for tokeni in lw_token:
                print("{} -> {} ".format(tokeni, lw_token))
                wv_index = wv_dict.get(tokeni, None)
                if wv_index is not None:
                    vector_t.append(wv_arr[wv_index].unsqueeze(0))
                else:
                    print("fail on {}".format(tokeni))
            if len(vector_t) > 0:
                vector_c = torch.cat(vector_t,0)
                #print(vector_c.size())
                vectors[i] = vector_c.mean(0)
                #print(vectors[i].cpu().data.numpy())
    return vectors
