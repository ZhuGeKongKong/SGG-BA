...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        pred_adj_np = np.load('./misc/conf_mat_transformer_train.npy')
        ...
        #pred_adj_np = pred_adj_np * 1.0
        #pred_adj_np = pred_adj_np + np.eye(pred_adj_np.shape[0])
        pred_adj_lap = adj_laplacian(pred_adj_np)
        self.pred_adj_lap = torch.from_numpy(pred_adj_lap).float().to(self.devices)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        # rel_dists_t = self.pred_adj_lap @ rel_dists.T
...
class TransformerTransferPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        obj_classes, rel_classes, att_classes = statistics['obj_classes'], statistics['rel_classes'], statistics[
            'att_classes']
        assert self.num_obj_cls == len(obj_classes), (self.num_obj_cls, len(obj_classes))
        assert self.num_att_cls == len(att_classes), (self.num_att_cls, len(att_classes))
        assert self.num_rel_cls == len(rel_classes), (self.num_rel_cls, len(rel_classes))
        self.val_alpha = config.MODEL.ROI_RELATION_HEAD.VAL_ALPHA
        ...
        if self.with_transfer:
            # print("!!!!!!!!!!!WITH_TRANSFER_CLASSIFIER!!!!!!!!!")
            # pred_adj_np = np.load('./misc/conf_mat_transformer_train.npy')
            # # pred_adj_np = 1.0 - pred_adj_np
            # pred_adj_np[0, :] = 0.0
            # pred_adj_np[:, 0] = 0.0
            ...
            print("!!!!!!!!!!!WITH_BiGraphTRANSFER_CLASSIFIER!!!!!!!!!")
            if len(rel_classes) == 51:
                fg_matrix = np.load("./misc/fg_matrix_overlap.npy")
            elif len(rel_classes) == 311:
                fg_matrix = np.load("./datasets/gqa/sceneGraphs/fg_matrix.npy")
            else:
                print("rel_classes is not match!!!")
            fg_matrix[0, :, :] = 0
            fg_matrix[:, 0, :] = 0
            fg_matrix[:, :, 0] = 0
            fg_matrix[0, 0, 0] = 1
            ...
            head_pred = pred_count_arg[:22]
            tail_pred = pred_count_arg[22:]
            ...
            bi_graph[:, 0] = 0
            bi_graph[0, 0] = 1.0
            bi_graph = bi_graph / (bi_graph.sum(0) + 1e-8)
            bi_graph = bi_graph + np.eye(bi_graph.shape[0])
            bi_graph = bi_graph / (bi_graph.sum(0) + 1e-8)
            self.pred_adj_lap = torch.from_numpy(bi_graph).float().to(self.devices)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.with_clean_classifier:
            ...
            rel_dists = rel_dists_clean
        if self.with_transfer:
            pred_adj_lap = self.pred_adj_lap
            #rel_dists = (pred_adj_lap @ rel_dists.T).T
            rel_dists = rel_dists @ pred_adj_lap
    ...
    def __init__(self, config, in_channels):
        ...
        if self.with_clean_classifier:
            ...
            if self.with_transfer:
                ...
                pred_count = np.transpose(pred_count)
                pred_count_arg = np.argsort(0.0 - pred_count)
                head_pred = pred_count_arg[:15]
                tail_pred = pred_count_arg[15:]
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.with_clean_classifier:
            ...
            if self.with_transfer:
                pred_adj_lap = self.pred_adj_lap
                # rel_dists = (pred_adj_lap @ rel_dists.T).T
                rel_dists_clean = rel_dists_clean @ pred_adj_lap
...
class CausalAnalysisPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        assert self.num_rel_cls == len(rel_classes)
        # init contextual lstm encoding
        #print("-------------------------:",config.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER)
        ...
        elif config.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER == "transformer":
            self.context_layer = TransformerContext(config, obj_classes, rel_classes, in_channels)
        else:
        ...
        self.with_clean_classifier = config.MODEL.ROI_RELATION_HEAD.WITH_CLEAN_CLASSIFIER
        if self.with_clean_classifier:
            self.ctx_compress_clean = nn.Linear(self.pooling_dim, self.num_rel_cls)
            self.vis_compress_clean = nn.Linear(self.pooling_dim, self.num_rel_cls)
            layer_init(self.ctx_compress_clean, xavier=True)
            layer_init(self.vis_compress_clean, xavier=True)
            self.freq_bias_clean = FrequencyBias(config, statistics)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.training:
            ...
            if self.with_clean_classifier:
                add_losses['auxiliary_ctx'] = F.cross_entropy(self.ctx_compress_clean(post_ctx_rep), rel_labels)
                if not (self.fusion_type == 'gate'):
                    add_losses['auxiliary_vis'] = F.cross_entropy(self.vis_compress_clean(union_features), rel_labels)
                    add_losses['auxiliary_frq'] = F.cross_entropy(self.freq_bias_clean.index_with_labels(pair_pred.long()),
                                                                  rel_labels)
            else :
    ...
    def calculate_logits(self, vis_rep, ctx_rep, frq_rep, use_label_dist=True, mean_ctx=False):
        if use_label_dist:
            if self.with_clean_classifier:
                frq_dists = self.freq_bias_clean.index_with_probability(frq_rep)
            else:
                frq_dists = self.freq_bias.index_with_probability(frq_rep)
        else:
            if self.with_clean_classifier:
                frq_dists = self.freq_bias_clean.index_with_labels(frq_rep.long())
            else:
        ...
        if self.with_clean_classifier:
            vis_dists = self.vis_compress_clean(vis_rep)
            ctx_dists = self.ctx_compress_clean(ctx_rep)
        else :
