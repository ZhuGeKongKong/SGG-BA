...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics, eps=1e-3, flag_binary=False):
        ...
        if flag_binary:
            pred_dist = torch.exp(pred_dist)
            pred_dist_1 = pred_dist[:,1:].sum(-1)
            pred_dist = torch.cat([pred_dist[:,0][:,None],pred_dist_1[:,None]],-1)
            pred_dist = torch.log(pred_dist / (pred_dist.sum(1)[:, None] + 1e-8) + 1e-8)
            self.obj_baseline = nn.Embedding(self.num_objs * self.num_objs, 2)
        else:
            self.obj_baseline = nn.Embedding(self.num_objs*self.num_objs, self.num_rels) #, sparse=True)
        with torch.no_grad():
...
class SpuerFrequencyBias(nn.Module):
    """
    The goal of this is to provide a simplified way of computing
    P(predicate | obj1, obj2, img).
    """

    def __init__(self, cfg, statistics, eps=1e-3, temp=1):
        super(SpuerFrequencyBias, self).__init__()
        # pred_dist = statistics['pred_dist'].float()
        # pred_dist = torch.exp(pred_dist)
        fg_matrix = statistics['fg_matrix'].float()
        #pred_dist = fg_matrix / (fg_matrix.sum(2)[:, :, None] + 1e-8)
        pred_dist = F.softmax(fg_matrix/temp, -1)
        assert pred_dist.size(0) == pred_dist.size(1)
...
class FrequencyInsBias(nn.Module):
    """
    The goal of this is to provide a simplified way of computing
    P(predicate | obj1, obj2, img).
    """

    def __init__(self, cfg, statistics, eps=1e-3, head=True, tail=False):
        super(FrequencyInsBias, self).__init__()
        pred_dist = statistics['pred_dist'].float()
        assert pred_dist.size(0) == pred_dist.size(1)
        ...
        pred_dist = torch.exp(pred_dist)
        if head:
            pred_dist = pred_dist.sum(1)
        if tail:
            pred_dist = pred_dist.sum(0)
        pred_dist = pred_dist/(pred_dist.sum(-1)[:,None]+1e-8)
        pred_dist = torch.log(pred_dist)
        self.obj_baseline = nn.Embedding(self.num_objs, self.num_rels)
        with torch.no_grad():
            self.obj_baseline.weight.copy_(pred_dist, non_blocking=True)
    ...
    def index_with_labels(self, labels):
        ...
        """
        if self.head:
            return self.obj_baseline(labels[:, 0])
        if self.tail:
            return self.obj_baseline(labels[:, 1])
