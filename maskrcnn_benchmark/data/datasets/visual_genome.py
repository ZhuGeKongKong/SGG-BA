...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_statistics(self, cfg=None):
        ...
        # np.save('./misc/fg_matrix_overlap.npy', fg_matrix)
        # a = 0
        # assert a==1
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True, cfg=None):
    ...
    train_data = VGDataset(split='train', img_dir=img_dir, roidb_file=roidb_file,
                           ...
                           filter_duplicate_rels=False, with_clean_classifier=False, cfg=cfg)
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap,
                ind_to_predicates=None, img_info=None, with_clean_classifier=False, cfg=None):
    """
    ...
    pred_num = 15
    pred_count = 0
    pred_max_sample = 9000

    rel_dist_prior = []
    pred_ij_dist = {}
    vg_dict = json.load(open('./datasets/vg/VG-SGG-dicts-with-attri.json','r'))
    ...
    if with_clean_classifier and split == 'train':
    ...
    if root_classes is not None and split == 'train':
        for pred_i in pred_ij_dist.keys():
            ...
            if len(pred_ij_dist[pred_i]) > pred_max_sample:
                pred_ij_dist[pred_i] = pred_ij_dist[pred_i][:pred_max_sample, :2]
            ...
            if split_mask[image_index[i]]:
                ...
                for j in range(len(rel_i)):
                    ...
                    if rel_j_pred in root_classes:
                        ...
                        if 0 in np.sum(np.abs(ij_ind[None,:]-pred_ij_dist[rel_j_pred]), -1):
