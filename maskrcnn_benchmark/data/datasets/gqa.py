...
class GQADataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        self.filenames_ori, self.img_info_ori = load_image_filenames(self.image_ids, split, img_dir, image_file)  # length equals to split_mask
        self.split_mask, self.gt_boxes, self.gt_classes, self.gt_attributes, self.relationships = load_graphs_nor(
            dict_file,
            self.image_ids,
            self.classes_to_ind, self.attributes_to_ind, self.predicates_to_ind,
            max_attribute_len=max_attribute_len,
            mode=self.split, num_val_im=num_val_im,
            filter_empty_rels=filter_empty_rels,
            ind_to_predicates=self.ind_to_predicates,
            filter_zeroshots=True,
            with_clean_classifier=with_clean_classifier,
            img_info=self.img_info_ori,
